...
This repo contains official agent skills for the [Runtype](https://runtype.com) platform, published via [skills.sh](https://skills.sh) and the Claude Code plugin marketplace.
...
Symlink or copy the skill into a project's `.claude/skills/` to test before pushing:

```bash
ln -s $(pwd)/skills/my-skill /path/to/project/.claude/skills/my-skill
```

Then open Claude Code in that project and invoke `/my-skill`.
