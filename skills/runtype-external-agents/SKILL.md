...
- Eval **capture** (`add_eval_case_from_execution`) needs a transcript on the run:
  ...
  never exports prompts or completions, so runs from it cannot be captured.
...
- `@flue/opentelemetry` → `instrument(createOpenTelemetryInstrumentation())`. Exports
  ...
  attributes server-side. Choose this for the improvement loop.
- `@runtypelabs/flue-otel` → `instrument(createRuntypeFlueInstrumentation())`. Exact
  iteration count, run-level stop reason, summed usage; no transcript (tool
  arguments/results only under its `content` opt-in). Choose this for cost and loop
  analytics.
